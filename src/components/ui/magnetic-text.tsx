// from 21st.dev — Morphing Cursor / Magnetic Text (adapté)
"use client";
...
interface MagneticTextProps {
  ...
  hoverText?: string;
  className?: string;
  /** Taille du disque qui suit la souris au hover (px) */
  circleSize?: number;
  /** Couleur du fond du disque (défaut : foreground) */
  circleBg?: string;
  /** Couleur du texte à l'intérieur du disque (défaut : background) */
  innerColor?: string;
}
...
export function MagneticText({
  ...
  return (
    ...
    >
      <span className="font-bold tracking-tighter leading-[0.9] text-foreground">
        {text}
      </span>
      ...
      {hoverText && (
        <div
          ...
          style={{
            ...
            backgroundColor: circleBg ?? "var(--foreground)",
            transition:
        ...
        >
          ...
          >
            <span
              className="font-bold tracking-tighter whitespace-nowrap leading-[0.9]"
              style={{ color: innerColor ?? "var(--background)" }}
            >
              {hoverText}
            </span>
          </div>
